...
class ModelCatalogProduct extends Model {
		...
		$query = $this->db->query("
		    SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (
		        ...
		        ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (
		            ...
		            OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' 
		            OR ps.date_end > NOW())) 
		            ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (
		                ...
		                AS reward, (
    		                ...
    		                AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (
    		                    ...
    		                    WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (
    		                        ...
    		                        WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (
		                                ...
		                                AS rating, (SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "review` r2 WHERE r2.product_id = p.product_id AND r2.status = '1' 
		                                GROUP BY r2.product_id) AS reviews, p.sort_order FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd 
		                                ...
		                                AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() 
	...
	public function getProductCategory() {
		...
		$query = $this->db->query("
		    SELECT imc_product.product_id, imc_product_to_category.category_id from imc_product
            JOIN imc_product_to_category 
            ON imc_product_to_category.product_id = imc_product.product_id
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				$sql .= " OR LCASE(p.ncm) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
